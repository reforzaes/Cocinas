...
const App = () => {
  ...
  const loadData = useCallback(async () => {
    ...
    try {
      ...
      const parsedIncidents = (iRes || []).map((inc: any) => {
        let history = [];
        // En MySQL/PHP, el campo history llegará como un STRING JSON
        if (inc.history && typeof inc.history === 'string' && inc.history.trim() !== '') {
          try {
            const parsed = JSON.parse(inc.history);
            history = Array.isArray(parsed) ? parsed : [];
          } catch (e) {
            console.warn("Error parseando history para ID:", inc.id, e);
            history = [];
          }
        } else if (Array.isArray(inc.history)) {
          history = inc.history;
        }
  ...
  const getSummary = (list: string[], type: 'seller' | 'installer') => list.map(label => {
    ...
    const totalSpecificIncidents = filteredIncidents.filter(i => 
      i.cause === targetCause && 
      (type === 'seller' ? i.assignedToSeller === label : i.assignedToInstaller === label || ks.some(k => k.id === i.kitchenId))
    );
    ...
    return { 
      ...
      incidencePercentage: totalKitchens > 0 ? (incidentsCount / totalKitchens) * 100 : 0 
    };
  });
  ...
  const handleAddIncident = async (newInc: any) => {
    setIsLoading(true);
    try {
      // Importante: history debe ir como string para MySQL
        ...
        <div className="max-w-7xl mx-auto flex justify-center gap-4">
          {['Portada', 'Tareas', 'Relación Proyectos'].map(tab => (
            <button 
              ...
              className={`px-8 py-3 rounded-2xl text-[10px] font-black uppercase tracking-widest transition-all active:scale-95 ${currentTab === tab ? 'bg-emerald-600 text-white shadow-xl' : 'text-gray-400 hover:bg-gray-50'}`}
            >
              {tab}
            </button>
          ))}
        ...
        {isLoading ? (
          ...
          <>
            ...
            {currentTab === 'Relación Proyectos' && (
              <KitchenList kitchens={filteredKitchens} incidents={incidents} />
            )}
          </>
        )}
      </main>
