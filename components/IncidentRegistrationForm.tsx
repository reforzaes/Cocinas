...
import { INSTALLERS, SELLERS, INCIDENT_CAUSES } from '../constants';
...
const IncidentRegistrationForm: React.FC<IncidentRegistrationFormProps> = ({
  ...
  const selectKitchen = (kitchen: Kitchen) => {
    ...
    setAssignedToSeller(kitchen.seller);
    setAssignedToInstaller(kitchen.installer);
    setShowResults(false);
  };
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    // Convertimos la observación inicial en la primera nota del historial
    ...
    onAddIncident({
      ...
      description,
      observation: observation.trim(), // Mantenido para legacy, pero usamos history
    ...
    <div className="bg-white p-8 rounded-[2rem] shadow-sm border mb-10">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
          ...
          <div>
            <label htmlFor="observation" className="block text-[10px] font-black text-gray-400 uppercase mb-2 tracking-widest">Observación Inicial (Follow-up)</label>
            <textarea
              id="observation"
              rows={4}
              className="w-full border-2 border-gray-100 rounded-2xl p-4 font-bold text-gray-700 focus:ring-4 focus:ring-emerald-100 outline-none transition-all"
