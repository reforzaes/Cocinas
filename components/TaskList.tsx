...
const TaskList: React.FC<TaskListProps> = ({ incidents, kitchens, onUpdateIncident }) => {
  ...
  const handleAddObservation = (incident: Incident) => {
    ...
    // Aseguramos que recuperamos el historial previo (si es string, lo parseamos, si no usamos el array)
          ...
          <div key={incident.id} className="bg-white p-8 rounded-[2.5rem] shadow-sm border border-gray-100 flex flex-col hover:shadow-xl transition-all group">
            ...
            <div className="flex-grow space-y-4 mb-8">
              ...
              <div className="bg-emerald-50/50 p-5 rounded-3xl border border-emerald-100/50 relative">
                <span className="text-[9px] font-black text-emerald-600 uppercase block mb-2">Última Gestión</span>
                ...
                {history.length > 1 && (
                  <button 
                    ...
                    className="absolute bottom-4 right-4 bg-emerald-600 text-white w-6 h-6 rounded-full flex items-center justify-center font-black text-xs shadow-lg hover:scale-110 transition-transform"
                ...
                {history.length === 1 && (
                  <button 
                    onClick={() => setShowHistoryId(incident.id)}
                    className="absolute bottom-4 right-4 text-emerald-600/50 hover:text-emerald-600 text-[8px] font-black uppercase tracking-tighter"
                  >
                    Histórico
                  </button>
                )}
            ...
            {incident.status !== TaskStatus.COMPLETED && (
              <div className="space-y-4">
                <textarea
                  ...
                  className="w-full border-2 border-gray-100 rounded-2xl p-3 text-[10px] font-bold text-gray-700 outline-none focus:border-emerald-500 min-h-[60px] bg-gray-50 focus:bg-white transition-all"
                />
                
                <div className="grid grid-cols-2 gap-3">
                  <button
                    onClick={() => handleAddObservation(incident)}
                    className="col-span-2 bg-gray-900 text-white py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-black transition-all shadow-lg shadow-gray-100"
                  >
                    Registrar Nota
                  </button>
                  ...
                  <button
                    ...
                    className="bg-emerald-600 text-white py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest hover:bg-emerald-700 transition-all shadow-lg shadow-emerald-100"
                  >
                    Completada
                  </button>
                </div>
          ...
          <div className="bg-white w-full max-w-xl rounded-[3rem] shadow-2xl overflow-hidden flex flex-col max-h-[80vh]">
            <div className="p-8 bg-gray-50 border-b flex justify-between items-center">
              <div>
                ...
                <p className="text-[10px] font-black text-gray-400 uppercase tracking-widest mt-1">Notas de seguimiento acumuladas</p>
              </div>
              <button onClick={() => setShowHistoryId(null)} className="text-3xl font-black text-gray-300 hover:text-red-500 transition-all">×</button>
            </div>
            
            <div className="p-8 overflow-y-auto space-y-6">
              {(Array.isArray(incidents.find(i => i.id === showHistoryId)?.history) ? incidents.find(i => i.id === showHistoryId)?.history : [])?.map((entry, idx) => (
                <div key={idx} className="flex gap-6 items-start group">
                  ...
                  <div className="flex-grow pb-6">
                    <div className="flex justify-between items-center mb-2">
                      <span className="text-[9px] font-black text-gray-300 uppercase">{new Date(entry.date).toLocaleString()}</span>
                    ...
                    <p className="text-xs font-bold text-gray-700 bg-white border border-gray-100 p-4 rounded-2xl shadow-sm">{entry.text}</p>
                  </div>
                </div>
              ))}
              {(!incidents.find(i => i.id === showHistoryId)?.history || incidents.find(i => i.id === showHistoryId)?.history?.length === 0) && (
                <p className="text-center text-gray-400 font-bold py-10">No hay historial registrado.</p>
              )}
            </div>
            ...
            <div className="p-8 border-t bg-gray-50 text-right">
              <button 
                ...
                className="bg-gray-900 text-white px-10 py-3 rounded-2xl text-[9px] font-black uppercase tracking-widest"
              >
                Cerrar Historial
              </button>
            </div>
